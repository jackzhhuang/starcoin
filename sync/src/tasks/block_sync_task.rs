            ...
            for parent_block_id in parent_block_ids {
                ...
                for child in &parent_block.children {
                        ...
                        })
                    {
                        ...
                        let executed_block = self
                            .chain
                            .apply_with_verifier::<DagBasicVerifier>(block_to_execute)?;
                        if executed_block.block().id() != block_to_execute_id {
                            bail!("The executed block id: {:?} is different from sync dag block id: {:?}", executed_block.block().id(), block_to_execute_id);
                        }
                        info!(
                            "succeed to apply a dag block: {:?}, number: {:?}",
                            executed_block.block.id(),
    ...
    fn execute_if_parent_ready(&mut self, children: Vec<HashValue>) -> Result<()> {
        ...
        for child in &children {
                ...
                })
            {
                let executed_block = self
                    .chain
                    .apply_with_verifier::<DagBasicVerifier>(block_to_execute.clone())?;
                if executed_block.block().id() != block_to_execute.id() {
                    bail!("The executed block id: {:?} is different from sync dag block id: {:?}", executed_block.block().id(), block_to_execute.id());
                }
                info!(
                    "succeed to apply a dag block: {:?}, number: {:?}",
                    executed_block.block.id(),
