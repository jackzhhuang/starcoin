...
fn test_verified_client_for_dag() {
    ...
    target_handle.set_dag_fork_number(TEST_FLEXIDAG_FORK_HEIGHT_FOR_DAG).expect("set fork number error");
    local_handle.set_dag_fork_number(TEST_FLEXIDAG_FORK_HEIGHT_FOR_DAG).expect("set fork number error");
